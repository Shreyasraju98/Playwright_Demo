...
test('Create Order-Update Order- Verify Order @smoke', async ({ page }) => {
  ...
  await page.locator('#ctl00_MainContent_fmwOrder_TextBox3').fill('Gujurat');
  await page.locator("#ctl00_MainContent_fmwOrder_UpdateButton").click()
  await page.waitForTimeout(3000);
  //Verify that City value change to Gujurat
  await expect(page.locator("//td[normalize-space()='"+ExpUserName+"']//following-sibling::td[text()='Gujurat']")).toHaveText("Gujurat")
